...
public interface ResourceBroker {
	 ...
	 * @param metric
	 * 			  The metric the listener listens to 
	 * @return a job object that is a handle to the task that was submitted
	 * @throws IOException
	 *             Upon non-remote IO problem
	 * @throws GATInvocationException
	...
	/**
	 * This operation indicates the start of a multicore job. A multicore job
	 * can run together with other multicore jobs on a single machine and 
	 * thereby make advantage of the single machine having multiple cores. 
	 * This operation should be followed by one or more submitJob operations 
	 * and an endMultiCoreJob operation. It should look like this:
	 * 
	 * broker.beginMultiCoreJob();
	 * job1 = broker.submitJob(jobDescription1);
	 * job2 = broker.submitJob(jobDescription2);
	 * ...
	 * jobN = broker.submitJob(jobDescriptionN);
	...
	/**
	 * This operation indicates the ending of a multicore job. After the
	 * invocation of this operation all submitted jobs after the last
	 * beginMultiCoreJob() are guaranteed to be submitted to a single
	 * machine. Note that in order to do so, an adaptor may delay the 
	 * submission of jobs that were submitted with submitJob, until this
	 * method is invoked.
	 * 
	 * @throws GATInvocationException
	 *             a remote problem occurred
