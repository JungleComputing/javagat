...
public class TestFileAdaptor {
	static void printUsage() {
		System.err
				.println("Usage: TestFileAdaptor command [argument] fileuri\n"
	...
	public static void main(String[] args) throws Exception {
		...
		CmdLineParser.Option cmdCreateFile = parser
				.addBooleanOption("createfile");
		CmdLineParser.Option cmdExists = parser.addBooleanOption("exists");
		CmdLineParser.Option cmdAbsolutePath = parser
				.addBooleanOption("absolutepath");
		...
		CmdLineParser.Option cmdSetReadOnly = parser
				.addBooleanOption("setreadonly");
		...
		if (parser.getOptionValue(cmdLastMod) != null) {
			System.out
					.println(location + " was last modified in milliseconds: "
							+ f.lastModified());
		}
		...
		if (parser.getOptionValue(cmdSetReadOnly) != null) {
			if (f.setReadOnly()) {
				System.out.println(location
						+ ": file attempted to set read only.");
			} else {
				System.out.println(location + ": operation failed.");
			}
		}
		...
		if (dest != null) {
			...
			System.out.println(location
					+ ": file attempted to copy to destination " + dest);
