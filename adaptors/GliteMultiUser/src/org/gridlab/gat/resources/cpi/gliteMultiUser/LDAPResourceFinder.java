...
/**
 * This class is used for querying LDAP servers for storage elements.
 * 
 * @author Stefan Bozic
 */
public class LDAPResourceFinder {
	...
	/** A Logger */
	private static final Logger LOGGER = LoggerFactory.getLogger(LDAPResourceFinder.class);

	/** The default BDII instance */
	...
	/** The default DN for LDAP queries */
	private static final String START_DN = "Mds-Vo-name=local,o=Grid";

	/** The LDAP context */
	private final DirContext ctx;

	/** Global Search controls for LDAP calls */
	private final SearchControls globalSearchControls;

	/** The preferred storage element id */
	private final String preferredSEID;

	/**
	 * Create a new LDAPResourceFinder with the given BDII.
	 * 
	 * @param gatContext GatContext with additional information.
	 * @param ldapResource URI to a BDII, if null a default value will be used.
	 * @throws NamingException if an LDAP error occurs.
	 */
	public LDAPResourceFinder(GATContext gatContext, URI ldapResource) throws NamingException {
	 ...
	 * 
	 * @param context GatContext with additional information.
	 * @throws NamingException if an LDAP error occurs.
	 */
	...
	/**
	 * Returns an attribute to an {@link SearchResult}.
	 * 
	 * @param result the search result
	 * @param name the attribute name
	 * @return an attribute to an {@link SearchResult}
	 * @throws NamingException an exception that might occurs
	 */
	 ...
	 * 
	 * @param voName name of the VO. Must not be null.
	 ...
	 * 
	 * @param voName name of the VO. Must not be null.
	 * @return a List&lt;String&gt; of LFC endpoints. Under normal circumstances this list should contain exactly one
	 *         entry.
	 * @throws NamingException if an LDAP error occurs.
	 */
	...
	public class SEInfo {
		...
		/** The id of the storage element */
		final String seUniqueId;

		/** The path to the storage element */
		final String path;
		 ...
		 * Constructor
		 * 
		 * @param id The id of the storage element
		 * @param pat The path to the storage element
		 * @param spac The space information
		 */
	...
	/**
	 * Retrieve a list of SEs for the given VO that have a minimal free space. The list is ordered in terms of
	 * preference:
	 * <ol>
	 * <li>The SE given by the {@link GliteConstants#PREFERENCE_PREFERRED_SE_ID} Gat context property.</li>
	 * <li>The SE given by the VO_VONAME_DEFAULT_SE environment variable</li>
	 * <li>All other SEs for the given VO.</li>
	 * </ol>
	 * 
	 * @param voName name of the VO. Must not be null.
	 * @param fileSize The minimal size that must be available in the SEs
	...
	public List<SEInfo> fetchSEs(final String voName, final long fileSize) throws NamingException {
		...
		findAndMoveSE(System.getenv("VO_" + voName.toUpperCase(Locale.ENGLISH) + "_DEFAULT_SE"), unsortedList,
				orderedList);
	...
	/**
	 * Copy a give StorageElement from a {@link List} to another {@link List}
	 * 
	 * @param seid the id of the storage element
	 * @param sourceList the source list woth the storage element
	...
	/**
	 * Returns a {@link List} with {@link SEInfo} to a given VO
	 * 
	 * @param voName the VO name
	 * @param fileSize the minimum filesize
	 * 
	 * @return a {@link List} with {@link SEInfo} to a given VO
	 * @throws NamingException an exception that might occurs
	 */
	private List<SEInfo> fetchAllSEs(final String voName, final long fileSize) throws NamingException {
		...
		while (searchResults.hasMore()) {
			...
			if (seUniqueId != null) {
				...
				LOGGER.debug("Found SE: " + seUniqueId + " with path " + path + " and space " + space + " Valid: "
						+ valid);
				if (valid) {
