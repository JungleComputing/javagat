...
public class SshFileAdaptor extends FileCpi {
	...
	protected URI correctURI(URI in) {
		if (in.getScheme() == null) {
			try {				return new URI("ssh:" + in.toString());
	...
	public void copy(URI loc) throws GATInvocationException {		
	    ...
	    if(location.getHost().equals(loc.getHost())) {
	    	 ...
	    	 *for the loc.getHost*/
	    	String destUserName = loc.getUserInfo();
	    	if(destUserName == null) {
	    		destUserName = (String) preferences.get("defaultUserName");
	    		if(destUserName == null) {
	    			destUserName = userName;
	    		}
	    	}
	    	if(destUserName.equals(userName)) {
		    	if (loc.getPath().equals(location.getPath())) {
		  		...
		  		copyOnSameHost(dest.getAbsolutePath());
		  			...
		  			if (GATEngine.DEBUG) {
						System.err.println("failed remote copy, source: user " + userName + " host " 
								+ location.getHost() + " dest: user " + destUserName + 
								" host " + loc.getHost());
						e.printStackTrace();
					}		  			
		  		}
	...
	protected void sendAck(OutputStream os) throws IOException {
		byte[] buf=new byte[1];
		// send '\0' (ACK)
		buf[0]=0; os.write(buf); os.flush();  	
  	}
  ...
  protected void doSingleUpload(URI loc) throws GATInvocationException {
	  ...
	  if(checkAck(in)!=0){	  		    	
	  	cleanSession(session, channel);
		throw new GATInvocationException("SshFileAdaptor: failed checkAck after sending scp command" + 
	  ...
	  try {
	  	...
	  	sendFileToRemote(f, in, out);	
	  		...
	  		cleanSession(session, channel);
			throw new GATInvocationException("SshFileAdaptor:doSingleUpload " 
												+ f.getPath() + " to " + loc + " failed; reason: " + ioe);
	  }		  
	  cleanSession(session, channel);
	  return;
	}
		...
		cleanSession(session, channel);
	   	throw new GATInvocationException("SshFileAdaptor: internal error: " + e + " in doSingleUpload " 
					+ f.getPath() + " to " + loc);
	}	
  }
  
  protected void sendDirectoryToRemote(File dir, InputStream in, OutputStream out) 
  						throws IOException {
  ...
  protected void sendDirectory(File lf, InputStream in, OutputStream out) 
	  ...
	  for (int entry = 0; entry < entries.length; entry++) {
	  		...
	  		if(current.isDirectory()) sendDirectoryToRemote(current, in, out);
	  		else sendFileToRemote(current, in, out);
        }
  }
  ...
  protected void doMultipleUpload(URI loc) throws GATInvocationException {
	  ...
	  session.connect();
		...
		try {
			...
			cleanSession(session, channel);
			throw new GATInvocationException("SshFileAdaptor:doMultipleUpload " 
											+ getPath() + " to " + loc + " failed; reason: " + ioe);
		}
		...
		cleanSession(session, channel);
		throw new GATInvocationException("SshFileAdaptor: internal error: " + e + " in doMultipleUpload " 
				+ f.getPath() + " to " + loc);
	}	
  }
  ...
  protected void scpFromLocalToRemote(URI loc) throws GATInvocationException {
	  try {
	  	if(!f.isDirectory()) doSingleUpload(loc);
	  	else doMultipleUpload(loc);
	  } catch(Exception e) {
	  		if (GATEngine.DEBUG) {
  ...
  private void fetchFile(File localFile,
        ...
        try {
            ...
            fos.close();
            if (GATEngine.DEBUG) {
        		System.err.println("scpFromRemoteToLocal: finished writing file"
        							+ localFile.getPath());
        	}
        }
    }
  ...
  protected void startRemoteCpProtocol(InputStream in, 
  									   OutputStream out, 
  									   File localFile) 
  									   throws IOException {
  	if (GATEngine.DEBUG) {
  	...
  	File startFile = localFile;
        while (true) {
            ...
            if (serverResponse.charAt(0) == 'C') {
            	if (GATEngine.DEBUG) {
        			System.err.println("scpFromRemoteToLocal: remote response is file");
        		}
                ...
                startFile = parseAndCreateDirectory(serverResponse,
                        startFile);
                if (GATEngine.DEBUG) {
        			System.err.println("scpFromRemoteToLocal: remote response is dir");
        		}
        		if(startFile != null)
                	sendAck(out);
                else throw new IOException("File " + localFile.getPath() + ": Not a directory");
            } else if (serverResponse.charAt(0) == 'E') {
                startFile = startFile.getParentFile();
                if (GATEngine.DEBUG) {
        			System.err.println("scpFromRemoteToLocal: remote response is E");
        		}
                sendAck(out);
            	...
            	if (GATEngine.DEBUG) {
    				System.err.println("remoteCpProtocol: read byte is none of the expected values");
    			}
            }            
        }  	
  }
		
  protected void scp(URI loc, boolean isRec) throws GATInvocationException {
  	/*-f from fetch*/
  	String command = "scp -f ";  	
    if (isRec) {
        command += "-r ";
        if (GATEngine.DEBUG) {
        	System.err.println("scpFromRemoteToLocal: remote is directory");
        }
    }
    command += getPath();  
    try {
    	session = jsch.getSession(userName, location.getHost(), 
  ...
  protected void scpFromRemoteToLocal(URI loc) throws GATInvocationException {  	
	...
	try {
		...
		if (isDir == TRUE) isRecursive = true; 
		scp(loc, isRecursive);			
	} catch(Exception e) {
	  		if (GATEngine.DEBUG) {
				System.err.println("SshFileAdaptor: for location " + location + " throws error " + e);
  ...
  protected void copyOnSameHost(String path) throws GATInvocationException {
		...
		case XOS :  if(isDir == TRUE) {
						((ChannelExec)channel).setCommand("cp -r " + getPath() + " " + path);
					}
					else {
						((ChannelExec)channel).setCommand("cp " + getPath() + " " + path);
					}
			    	err = ((ChannelExec)channel).getErrStream();    			    
    			    channel.connect();
    			    while(true) {
		...
		case WOS :  if(isDir == TRUE) {
						((ChannelExec)channel).setCommand("copy " + toMW(getPath()) + " " + toMW(path));
					}
					else {
						((ChannelExec)channel).setCommand("xcopy /I/E/H/K/O/Q/Y " + toMW(getPath()) + " " + toMW(path));
					}
			    	err = ((ChannelExec)channel).getErrStream();    			    
    			    channel.connect();
    			    while(true) {
  ...
  protected int checkAck(InputStream in) throws IOException{
	...
	private boolean deleteLocalRecursively(File localFile) {
		if(!localFile.isDirectory()) return localFile.delete();
		File[] entries = localFile.listFiles();
		boolean result = true;
		for(int i=0; i < entries.length; i++) {
			if(entries[i].isDirectory()) result = result && deleteLocalRecursively(entries[i]);
			else result = result && entries[i].delete();
		}
	...
	private boolean deleteByForceRecursively() {
		...
		case XOS :  if(isDir == TRUE)
    			    ...
    			    return true;
		case WOS :  if(isDir == TRUE)
						((ChannelExec)channel).setCommand("rmdir /S /Q " + toMW(getPath()));
					else 
						((ChannelExec)channel).setCommand("del /q " + toMW(getPath()));
			    	err = ((ChannelExec)channel).getErrStream();
    			    channel.connect();
...
//		String uriString = location.toString();		
	...
	public boolean renameTo(java.io.File arg0) throws GATInvocationException {		
    	...
    	copy(destination);
		if(!deleteByForceRecursively()) return false;	     	
		try{
	     	updateLocation(destination);
	     } catch(GATObjectCreationException e) {
	     	throw new GATInvocationException
	...
	private void updateLocation(URI dest) throws GATObjectCreationException {
		if(isLocalFile) {
			...
			if(dest.isLocal()) isLocalFile = true;
 		} 
