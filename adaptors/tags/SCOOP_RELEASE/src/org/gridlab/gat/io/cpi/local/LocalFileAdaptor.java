...
public class LocalFileAdaptor extends FileCpi {
	...
	protected URI correctURI(URI in) {
	    if (in.getScheme() == null) {
			try {
				return new URI(URIEncoder.encodeUri("file:///" + in.getPath()));
...
*/
		...
		if (tmp.isDirectory()) {
			...
			try {
				loc = new URI(URIEncoder.encodeUri(u));
			} catch (URISyntaxException e) {
	...
	public org.gridlab.gat.io.File getAbsoluteFile()
		...
		try {
			return GAT.createFile(gatContext, preferences, localToURI(f.getAbsoluteFile().getPath()));
		} catch (Exception e) {
			throw new GATInvocationException("default file", e);
		}
	}
	...
	public org.gridlab.gat.io.File getCanonicalFile()
			throws GATInvocationException {
		try {
			return GAT.createFile(gatContext, preferences, localToURI(f.getCanonicalFile().getPath()));
		} catch (Exception e) {
			throw new GATInvocationException("default file", e);
		}
	}
	...
	public org.gridlab.gat.io.File getParentFile()
			throws GATInvocationException {
		try {
			return GAT.createFile(gatContext, preferences, localToURI(f.getParentFile().getPath()));
		} catch (Exception e) {
			throw new GATInvocationException("default file", e);
		}
	}
	...
	public org.gridlab.gat.io.File[] listFiles() throws GATInvocationException {
		...
		for (int i = 0; i < r.length; i++) {
			try {
				res[i] = GAT.createFile(gatContext, preferences, localToURI(r[i].getPath()));
			} catch (Exception e) {
				throw new GATInvocationException("default file", e);
			}
		}
    ...
    public URI localToURI(String path) throws URISyntaxException {
        return new URI(URIEncoder.encodeUri(path.replace(File.separatorChar, '/')));
    }
