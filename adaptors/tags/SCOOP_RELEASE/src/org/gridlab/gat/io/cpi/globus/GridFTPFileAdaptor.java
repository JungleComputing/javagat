...
public class GridFTPFileAdaptor extends GlobusFileAdaptor {
    ...
    public GridFTPFileAdaptor(GATContext gatContext, Preferences preferences,
        ...
        try {
            GlobusSecurityUtils.getGlobusCredential(gatContext, preferences, "gridftp", location, DEFAULT_GRIDFTP_PORT);
        } catch (Exception e) {
...
//        c.setMode(GridFTPSession.MODE_BLOCK);
     ...
     */
    protected FTPClient createClient(GATContext gatContext, Preferences preferences, URI hostURI)
    ...
    private static String getClientKey(URI hostURI, Preferences preferences) {
        return hostURI.resolveHost() + ":" + hostURI.getPort(DEFAULT_GRIDFTP_PORT) + preferences; // include preferences in key
    }
    
    protected static GridFTPClient doWorkCreateClient(
            GATContext gatContext, Preferences preferences, URI hostURI) throws GATInvocationException {
        try {
            GSSCredential credential = GlobusSecurityUtils.getGlobusCredential(gatContext, preferences, "gridftp", hostURI, DEFAULT_GRIDFTP_PORT);
    ...
    protected void destroyClient(FTPClient c, URI hostURI, Preferences preferences) {
        ...
        if(USE_CLIENT_CACHING && !clienttable.containsKey(key)) {
            ...
            try {
                if (GATEngine.DEBUG) {
                    System.err
                            .println("closing gridftp client");
                }
                c.close();
            } catch (Exception e) {
                if (GATEngine.DEBUG) {
