...
//			Stub stub = (Stub) sbox;
	...
	public void add(Advertisable advert, MetaData metaData, String path)
		...
		try {
			sbox.createObject(NAMESPACE, USR_OBJECT_PREFIX+absolutePath(path));
			sbox.createAttribute(NAMESPACE, OBJ_STATE_ATTRIBUTE, TYPE_STRING, true);
			tmp=GATEngine.getGATEngine().marshalAdvertisable(advert);
			sbox.setAttribute(NAMESPACE, USR_OBJECT_PREFIX+absolutePath(path), OBJ_STATE_ATTRIBUTE, tmp);

			for (int i = 0; i < metaData.size(); i++) {
				sbox.createAttribute(NAMESPACE, USR_ATTRIBUTE_PREFIX+metaData.getKey(i),
						TYPE_STRING, true);
				sbox.setAttribute(NAMESPACE, USR_OBJECT_PREFIX+absolutePath(path), USR_ATTRIBUTE_PREFIX+metaData.getKey(i), metaData
						.getData(i));
			}
		} catch (Exception e) {
	...
	public void delete(String path) throws NoSuchElementException,
		...
		try {
			sbox.deleteObject(NAMESPACE, USR_OBJECT_PREFIX+absolutePath(path));
		} catch (Exception e) {
			throw new GATInvocationException("storagebox", e);
		}
	}
	...
	public Advertisable getAdvertisable(String path)
		...
		try {
			...
			StringSet objects = createStringSet(USR_OBJECT_PREFIX+absolutePath(path));
			StringSet attributes_names = createStringSet(OBJ_STATE_ATTRIBUTE);

			ObjectSet os=sbox.getAttributes(namespaces, objects, attributes_names);
			...
			return GATEngine.getGATEngine().unmarshalAdvertisable(gc, prefs, tmp);
		} catch (Exception e) {
			throw new GATInvocationException("storagebox", e);
		}
	}
	...
	public String[] find(MetaData metaData) throws GATInvocationException {
		...
		for (int i = 0; i < metaData.size(); i++) {
			if (!isRegexp(metaData.getKey(i))) {
				query += USR_ATTRIBUTE_PREFIX+metaData.getKey(i) + "=" + metaData.getData(i);
				if(i<metaData.size()-1) query+="&";
			} else {
				query += USR_ATTRIBUTE_PREFIX+metaData.getKey(i) + "~" + metaData.getData(i);
                                if(i<metaData.size()-1) query+="&";
			}
		...
		try {
			...
			ObjectSet res=sbox.query(createStringSet(NAMESPACE), query, createStringSet(""));

			StorageBoxObject[] sbo = res.getItem();

			if(sbo.length == 0) return null;
		...
		try {
			...
			for (int i = 0; i < vals.length; i++)
			{
				if(!vals[i].getAttribute_name().equals(OBJ_STATE_ATTRIBUTE))
					res.put(extractMetaDataName(vals[i].getAttribute_name()), vals[i].getAttribute_value());
			}
