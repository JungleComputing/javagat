...
{
	long frequency=0;
	Map measurees = java.util.Collections.synchronizedMap(new HashMap());
	...
	public synchronized void addMeasuree(Metric m, MetricInstance mi)
	{
		if(!measurees.containsKey(m))
		{
			measurees.put(m, new Measuree(mi, m.getFrequency()));
                        ...
                        else frequency = (new BigInteger(new Long(frequency).toString())).gcd(new BigInteger(new Long(m.getFrequency()).toString())).intValue();
		}
	}

	private long computeFrequency()
	{
                if(measurees.size()==0) frequency=0;
                else
		{
			Object[] metricsSet = measurees.keySet().toArray();
			BigInteger Frequency=new BigInteger(new Long( ( (Metric) metricsSet[0]).getFrequency()).toString() );
			for(int i=1;i<metricsSet.length;i++)
			{
				Frequency=Frequency.gcd(new BigInteger(new Long( ((Metric) metricsSet[i]).getFrequency()).toString() ));
			}
		...
		for(int i=0;i<measureesSet.length;i++)
		{
			...
			if(m.timeLeft <= 0)			
			{
				...
				MonitorConsumer.CommandResult cmdres = mi.get();
                        	...
                        	if (cmdres.hasFailed())
                        	{
                                	System.err.println("The GET command failed: " + cmdres.getStatusStr());
                        	}
			}
			else m.timeLeft -= frequency;
		}
	}
...
class LocalMetricListener implements hu.sztaki.lpds.monitor.MetricListener
{
	...
	{
		hu.sztaki.lpds.monitor.MetricDefinition md_mercury=value.getDefinition();

	//System.out.println("Measurement type: "+md_mercury.getMeasurementType().toString());
                 ...
                 if(index>-1)
                 {
                        MonitorizedMetric mm = (MonitorizedMetric) mma.monitorizedMetrics.get(index);
                        for(int i=0;i<mm.getListeners().size();i++)
                                ((MetricListener)(mm.getListeners().get(i))).ProcessMetricEvent(new MetricValue("localhost", value.getValue(), mm.metric, value.getTimeStamp().getTime()));
                 }
		else if(md_mercury.getMeasurementType().toString().equals("continuous"))
		{
...
{
	public Vector monitorizedMetrics=new Vector();
	//public hu.sztaki.lpds.monitor.MetricValue crt_mv=null;

	public Map results = java.util.Collections.synchronizedMap(new HashMap());

	public Map definitions = java.util.Collections.synchronizedMap(new HashMap());	
	...
	public String producer_URL="monp://localhost:3571";
			...
			Preferences preferences) throws GATObjectCreationException
	{
		...
		{
			System.err.println("Mercury monitoring adaptor: Exception occured when performing authentication at producer at the following URL:"+ producer_URL + ":");
			e.printStackTrace();
		}

		if(resp.hasFailed())
		{
			System.err.println("Mercury monitoring adaptor: Authentication to producer at the following URL:"+ producer_URL +" has failed.");
		}
...
*/
	...
	public MonitorizedMetric lookupMonitorizedMetricByMetric(Metric metric)
	{
		...
		{
			if(((MonitorizedMetric) monitorizedMetrics.get(i)).getMetric().equals(metric)) return (MonitorizedMetric) monitorizedMetrics.get(i);
		}
			...
			throws GATInvocationException
	{
		...
		if(metric.getDefinition().getMeasurementType() == MetricDefinition.DISCRETE || metric.getFrequency() > 0)
		{
			...
			if(metric.getMetricParameters()!=null)
			{
				...
				{
					args[i] = new MonitorArg((String) ((Map.Entry)entries[i]).getKey(), ((Map.Entry)entries[i]).getValue());
				}
			}

			CollectResult collectres = null;

			if(args==null) collectres = mc.collect(metric.getDefinition().getMetricName());
			else collectres = mc.collect(metric.getDefinition().getMetricName(), args);
			...
			{
				System.err.println("The COLLECT command failed: " + collectres.getStatusStr());
			}

			MonitorConsumer.MetricInstance metricInstance = collectres.getMetricInstance();
			if(metric.getDefinition().getMeasurementType() == MetricDefinition.DISCRETE)
			{
				/* Send a SUBSCRIBE command for the instance. */
				MonitorConsumer.CommandResult cmdres = metricInstance.subscribe();
				/* Wait for the result. */
				try
				{
					cmdres.waitResult();
			...
			else
			{
                                ...
                                if(!trigger.isAlive()) trigger.start();
			...
			Metric metric) throws GATInvocationException
	{
		...
		if(mm1!=null)
		{
			mm1.removeListener(metricListener);
	                ...
	                // if there are no more listeners for this metric, stop monitorizing the metric:
			if(mm1.getListeners().size()==0)
			{
				if(mm1.metricInstance != null)
				{
		                        MonitorConsumer.CommandResult cmdres = mm1.metricInstance.stop();
                        		/* Wait for the result. */
					try
					{
                        			cmdres.waitResult();
                        		...
                        		if (cmdres.hasFailed())
                        		{
                                		System.err.println("The STOP command failed: " + cmdres.getStatusStr());
                        		}
				}
				if(metric.getDefinition().getMeasurementType()==MetricDefinition.CONTINUOUS && metric.getFrequency()>0)
				{
	...
	{
		if(metricNames == null) initializeMetricNames();
		...
		// query the Mercury monitor for metric definitions for which the definition is not yet known, using the "monitor.metric.def" predefined metric:

		int no_queried = 0;

		for(int i=0;i<metricNames.length;i++)
		{
			if(definitions.containsKey(metricNames[i])) continue;
		...
		for(int i=0;i<no_queried;i++)
		{
			...
			queries[i].waitResult(MAX_BLOCK_TIME);
                	...
                	//System.out.println("queries["+i+"].getResultId() ="+queries[i].getResultId());

                	int status = queries[i].getStatus();

                	if(status!=0)
                	{
                        	System.err.println("Query for metric "+"monitor.metric.def"+" has failed. Status returned ="+queries[i].getStatusStr());
                	}

                	if(status!=0 || queries[i].hasFailed()) continue;
		...
		{
	                // look for result placed in the "results" HashMap by the listener (indexed by result ID):

			responses[i] = (hu.sztaki.lpds.monitor.MetricValue) results.get(responded[i]);
		...
		String metricType = null, metricUnit = null;
		...
		for(int i=0;i<responses.length;i++)
		{
                	...
                	metricType = null; metricUnit = null;
                	...
                	if(index_aux > -1 || index_aux1 > -1) // type is record type or enum type
                	{
                	...
                	else if(index > -1) // type is not record type and unit is given
                	{
                        	...
                        	if(index2 > -1)
                        	{
                                	...
                                	if(index3 > -1) metricUnit = metricTypeUnit.substring(index2+1, index3);
                        	}
                	}
                	...
                	Map param_defs = new HashMap();

                	for(int j=0;j<metricParams.length;j++)
                        	param_defs.put( ( (Object[]) metricParams[j])[0],  ( (Object[]) metricParams[j])[1]);

                	int measurementType = MetricDefinition.CONTINUOUS;
                	if(!metricMeasurement.equals(new Integer(0))) measurementType = MetricDefinition.DISCRETE;

                	System.out.println(metricName+", "+measurementType+", "+  metricType +", "+metricUnit+", "+param_defs+", "+"null");

                	MetricDefinition rv = new MetricDefinition(metricName, measurementType, metricType , metricUnit, param_defs, null);

                	if(rv!=null) definitions.put(metricName, rv);
		}
			...
			throws GATInvocationException
	{
		...
		if(metric.getDefinition() == null) return null;
		...
		CommandResult result = mc.query(metric.getDefinition().getMetricName(), args);
		...
		if(status!=0)
		{
			System.err.println("Query for metric "+metric.getDefinition().getMetricName()+" has failed. Status returned ="+result.getStatusStr());
			return null; // am modificat aici (adaugat) -> ev sursa de erori
		}

		if(result.hasFailed()) return null;

		Integer id = new Integer(result.getResultId());

		// look for result placed in the "results" HashMap by the listener (indexed by result ID):

		hu.sztaki.lpds.monitor.MetricValue crt_mv1 = (hu.sztaki.lpds.monitor.MetricValue) results.get(id);
		...
		return new MetricValue("localhost", crt_mv1.getValue(), metric, crt_mv1.getTimeStamp().getTime());
	}
			...
			throws GATInvocationException
	{
		...
		if(definitions.containsKey(name)) return (MetricDefinition) definitions.get(name);

		// ... or else produce it now, using a query of the standard "monitor.metric.def" metric:

		MetricDefinition md = new MetricDefinition("monitor.metric.def", MetricDefinition.CONTINUOUS, "", "", null, null);
		...
		else // type is not record type and unit is not given
		{
        	...
        	for(int i=0;i<metricParams.length;i++)
                	param_defs.put( ( (Object[]) metricParams[i])[0],  ( (Object[]) metricParams[i])[1]);
		...
		if(rv!=null) definitions.put(name, rv);


/*
		if(name.equals("host.cpu.number")) return new MetricDefinition("host.cpu.number", MetricDefinition.CONTINUOUS, "dataTypeInt", "pieces", null, null);
		if(name.equals("app.message")) return new MetricDefinition(name, MetricDefinition.DISCRETE, "dataTypeInt", "", null, null);

		if(name.equals("app.priv.Mercury-Example-Java-Appmonitor-1.processed")) return new MetricDefinition(name, MetricDefinition.DISCRETE, "dataTypeInt", "", null, null);
*/
