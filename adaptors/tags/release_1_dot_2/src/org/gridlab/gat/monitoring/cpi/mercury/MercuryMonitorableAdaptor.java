...
class MeasurementTrigger extends Thread {
    ...
    public synchronized void addMeasuree(Metric m, MetricInstance mi) {
        if (!measurees.containsKey(m)) {
            ...
            } else frequency = (new BigInteger(new Long(frequency).toString()))
                .gcd(new BigInteger(new Long(m.getFrequency()).toString()))
                .intValue();
        }
    }
            ...
            for (int i = 0; i < mm.getListeners().size(); i++)
                ((MetricListener) (mm.getListeners().get(i)))
                    .processMetricEvent(new MetricValue("localhost", value
                        .getValue(), mm.metric, value.getTimeStamp().getTime()));
...
public class MercuryMonitorableAdaptor extends MonitorableCpi {
    ...
    public Map definitions = java.util.Collections
    ...
    public void addMetricListener(MetricListener metricListener, Metric metric)
        ...
        if (metric.getDefinition().getMeasurementType() == MetricDefinition.DISCRETE
            ...
            if (args == null) collectres = mc.collect(metric.getDefinition()
                .getMetricName());
            else collectres = mc.collect(
                metric.getDefinition().getMetricName(), args);
    ...
    public List getMetricDefinitions() throws GATInvocationException {
        ...
        for (int i = 0; i < responses.length; i++) {
            ...
            for (int j = 0; j < metricParams.length; j++)
                param_defs.put(((Object[]) metricParams[j])[0],
    ...
    public MetricValue getMeasurement(Metric metric)
        ...
        for (int i = 0; i < arg.size(); i++) {
            args[i] = new MonitorArg(
                (String) ((Map.Entry) entries[i]).getKey(),
                ((Map.Entry) entries[i]).getValue());
        }
    ...
    public MetricDefinition getMetricDefinitionByName(String name)
        ...
        for (int i = 0; i < metricParams.length; i++)
            param_defs.put(((Object[]) metricParams[i])[0],
        ...
        /*
         ...
         + metricType + ", " + metricUnit + ", " + param_defs + ", "
         + "null");
         */
