 ...
 * @see MonitorType
 * @author G??bor Gomb??s
 * @version $Id: Buffer.java,v 1.2 2005/10/07 11:05:55 rob Exp $
 *
...
public class Buffer {
    ...
    public void ensureFree(int needed) {
        ...
        if (end + needed < data.length) return;
    ...
    public void encode(Object value, MonitorType type) {
        if (type instanceof ArrayType) encodeArray((Object[]) value,
            (ArrayType) type);
        else if (type instanceof RecordType) encodeRecord((Object[]) value,
            (RecordType) type);
        else if (type instanceof SimpleType) encodeSimple(value,
            (SimpleType) type);
        else throw new IllegalArgumentException("Unknown type definition");
    }
    ...
    private void encodeArray(Object[] value, ArrayType type) {
        ...
        if (type.getLength() < 0) addInt(value.length);
    ...
    public Object decode(MonitorType type) {
        if (type instanceof ArrayType) return decodeArray((ArrayType) type);
        else if (type instanceof RecordType) return decodeRecord((RecordType) type);
        else if (type instanceof SimpleType) return decodeSimple((SimpleType) type);
        else throw new IllegalArgumentException("Unknown type definition");
    }
