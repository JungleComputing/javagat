 ...
 *
 * @author G??bor Gomb??s
 * @version $Id: MonitorConsumer.java,v 1.2 2005/10/07 11:05:55 rob Exp $
 *
...
public class MonitorConsumer extends Thread {
    ...
    private void metricCallback(int mid, long timestamp, byte[] value) {
        ...
        if (buf == null) buf = new Buffer(value);
        else buf.setData(value);
    ...
    private void checkPendingException() {
         ...
         * a runtime exception */
        if (pendingException instanceof RuntimeException) throw (RuntimeException) pendingException;
        else throw new MonitorConnectionException(pendingException);
    }
    ...
    public void run() {
        try {
            ...
            if (metricListeners.size() == 0) checkPendingException();
     ...
     */
    private synchronized native MetricDefinition nativeGetMetricDefinition(
        int metricId) throws UnknownMetricException;
