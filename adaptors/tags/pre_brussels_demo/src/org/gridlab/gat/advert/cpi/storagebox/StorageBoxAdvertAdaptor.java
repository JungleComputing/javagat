...
/*
	...
	public void add(Advertisable advert, MetaData metaData, String path)
		...
		try {
			...
			sbox.setAttribute(NAMESPACE, path, ATTRIBUTE, GATEngine.getGATEngine().marshalAdvertisable(advert));
	...
	public Advertisable getAdvertisable(String path)
		...
		try {
			...
			if(vals.length == 0) {
				...
				throw new Error("Internal error in storagebox adaptor, found multiple objects at path");
			}
			
				AttributeValue v = vals[0];
				return GATEngine.getGATEngine().unmarshalAdvertisable(v.getAttribute_value());
		} catch (Exception e) {
			throw new GATInvocationException("storagebox", e);
		}
	}
	...
	public String[] find(MetaData metaData) throws GATInvocationException {
		...
		try {
			AttributeSet res = sbox.query(createStringSet(NAMESPACE), query, createStringSet(""));		
	...
	public MetaData getMetaData(String path) throws GATInvocationException,
		...
		try {
			StringSet namespaces = createStringSet(NAMESPACE);
			StringSet objects = createStringSet(path);
			StringSet attributes_names = createStringSet(""); // means all attributes
