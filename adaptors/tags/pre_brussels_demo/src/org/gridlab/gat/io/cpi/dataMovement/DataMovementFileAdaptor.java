...
public class DataMovementFileAdaptor extends FileCpi {
	...
	protected static URI fixURI(URI in) {
		try {
			...
			if(scheme == null) {
				return new URI("gsiftp://" + s);
			} else if(scheme.equals("gsiftp")) {
				return in;
			} else {
				int index = s.indexOf(':');
				return new URI("gsiftp:" + s.substring(index+1, s.length()));
			}
	...
	public void copy(URI dest) throws GATInvocationException, IOException,
		...
		if(GATEngine.VERBOSE) {
			System.err.println("copying from " + sourceURL + " to " + destURL);
		}
	...
	public void move(URI dest) throws IOException, GATInvocationException {
		...
		res = data.DATAMoveFileDefaults(fixURI(toURI()).toString(), fixURI(dest)
				.toString());
