...
public class DefaultResourceBrokerAdaptor extends ResourceBrokerCpi {
	...
	public Job submitJob(JobDescription description) throws GATInvocationException,
		...
		String command = getLocation(description) + getArguments(description);
		...
		// read the output to avoid deadlocks
		OutputForwarder error = new OutputForwarder(p.getErrorStream(), true);
