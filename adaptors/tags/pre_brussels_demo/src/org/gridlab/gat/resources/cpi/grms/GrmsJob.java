...
public class GrmsJob extends Job {
	 ...
	 */
	public void unSchedule() throws GATInvocationException, RemoteException, IOException {
	...
	private Map createInfoMap(JobInformation jobInfo) {
		...
		if(localSubTime != null) {
			res.put("local_submissiontime", "" + localSubTime.getTimeInMillis());
		}
	...
	private void updateState(String status) {
		if (status.equals(JobStatusType._QUEUED)) {
			...
			System.err
					.println("Internal error in grms adaptor, unknown state returned: " + status);
		}
	}
	 ...
	 */
	public void checkpoint() throws GATInvocationException, RemoteException,
			IOException {
