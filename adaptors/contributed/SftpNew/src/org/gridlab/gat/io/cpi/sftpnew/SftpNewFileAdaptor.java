...
public class SftpNewFileAdaptor extends FileCpi {
     ...
     * @param location
     */
    public SftpNewFileAdaptor(GATContext gatContext, URI location)
    ...
    protected static SftpNewConnection createChannel(GATContext gatContext,
        ...
        if (!USE_CLIENT_CACHING) {
            return doWorkCreateChannel(gatContext, location);
        }
        ...
        if (c == null) {
            c = doWorkCreateChannel(gatContext, location);
        }
    ...
    private static SftpNewConnection doWorkCreateChannel(GATContext gatContext,
        ...
        try {
            sui = SftpSecurityUtils.getSshCredential(gatContext, "sftp",
                    location, SSH_PORT);
        } catch (Exception e) {
            if (logger.isDebugEnabled()) {
    ...
    protected void copyToLocal(URI src, URI dest) throws GATInvocationException {
        SftpNewConnection c = createChannel(gatContext, src);
    ...
    protected void copyToRemote(URI src, URI dest)
        ...
        try {
            ...
            tmpCon = createChannel(gatContext, dest);
    ...
    public long length() throws GATInvocationException {
        SftpNewConnection c = createChannel(gatContext, location);
        try {
