...
public class SshSlurmJob extends SimpleJobBase implements MetricListener {
    
    protected static Logger logger = LoggerFactory.getLogger(SshSlurmJob.class);
    ...
    protected SshSlurmJob(GATContext gatContext,
	...
	subContext = SshSlurmResourceBrokerAdaptor.getSubContext(gatContext);
    }
    ...
    public SshSlurmJob(GATContext gatContext, SerializedSimpleJobBase sj)
	...
	try {
	    jobHelper = SshSlurmResourceBrokerAdaptor.subResourceBroker(subContext, brokerURI);
	} catch (URISyntaxException e) {
	    throw new GATObjectCreationException("Could not create broker to get Slurm job status", e);
	}
	startListener();
    }
    ...
    public static Advertisable unmarshal(GATContext context, String s)
	    throws GATObjectCreationException {
	return SimpleJobBase.unmarshal(context, s, SshSlurmJob.class.getClassLoader());
    }
    ...
    protected void getJobState(String jobID) throws GATInvocationException {
        ...
        try {
            ...
            try {
                ...
                sd.setArguments("-c", "squeue -h -o %t -j " + jobID);
                ...
                resultState = mapSlurmStatetoGAT(status);
            } catch (IOException e) {
    ...
    private JobState mapSlurmStatetoGAT(String slurmState) {

	if (slurmState == null) {
	    logger.error("Error in mapSlurmStatetoGAT: no SlurmState returned");
		...
		if (missedJob > 1) {
		    ...
		    // submission of the job and its appearance in squeue output. But it may
		...
		return state;
	        ...
	        if (slurmState.equals("NF")) {
	            // node fail
	            return JobState.SUBMISSION_ERROR;
	        }
	        if (slurmState.equals("CA") || slurmState.equals("CD") || slurmState.equals("F") || slurmState.equals("TO")) {
	            // slurmState cancelled, completed, failed, timeout
	            return JobState.STOPPED;
	        }
	        if (slurmState.equals("CF")) {
	            // configuring
	            return JobState.PRE_STAGING;
	        }
	        if (slurmState.equals("CG")) {
	            // completing
	            return JobState.POST_STAGING;
	        }
	        if (slurmState.equals("PD")) {
	            // pending
	            return JobState.SCHEDULED;
	        }
	        if (slurmState.equals("R")) {
	            // pending
	            return JobState.RUNNING;
	        }
	        if (slurmState.equals("S")) {
	            // pending
	...
	try {
	    ...
	    logger.debug("SshSlurm adaptor: exit value file " + returnValueFile
		    + " not found!");
	    return null;
	} catch (IOException e) {
	    return null;
