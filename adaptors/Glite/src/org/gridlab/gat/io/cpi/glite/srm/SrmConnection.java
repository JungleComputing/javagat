...
import gov.lbl.srm.StorageResourceManager.TSupportedTransferProtocol;
...
public class SrmConnection {
     ...
     */
    public SrmConnection(String host, final String proxyPath)
            throws IOException {
        // Set provider
        ...
        try {
            ...
            ((Stub) this.service)._setProperty(GSIConstants.GSI_CREDENTIALS,
                    gssCredential);
            ((Stub) this.service)._setProperty(GSIConstants.GSI_AUTHORIZATION,
                    NoAuthorization.getInstance());
            ((Stub) this.service)._setProperty(GSIConstants.GSI_MODE,
        ...
        SrmGetTransferProtocolsResponse result = service
                .srmGetTransferProtocols(new SrmGetTransferProtocolsRequest());
        ...
        if (result.getProtocolInfo() != null) {
            TSupportedTransferProtocol[] arrayProtocols = result
                    .getProtocolInfo().getProtocolArray();
            for (int i = 0; i < arrayProtocols.length; i++) {
                // System.out.println("\t"+arrayProtocols[i].getTransferProtocol());
                if (arrayProtocols[i].getTransferProtocol().equals("gsiftp")) {
                    protocol = "gsiftp";
                }
            }
        }
    ...
    public String getTURLForFileDownload(String uriSpec) throws IOException {
        ...
        if (!s.equals(TStatusCode.SRM_REQUEST_INPROGRESS)
                && !s.equals(TStatusCode.SRM_REQUEST_QUEUED)
                && !s.equals(TStatusCode.SRM_SUCCESS)) {
        ...
        if (!status.getStatusCode().equals(TStatusCode.SRM_SUCCESS)) {
            ...
            do {
                ...
                retryCount++;
            } while ((!(status.getStatusCode().equals(TStatusCode.SRM_DONE) || status
    ...
    public SRMPosixFile ls(String uri) throws IOException {
        ...
        srmLsRequest.setArrayOfSURLs(new ArrayOfAnyURI(
                new URI[] { new URI(uri) }));
        ...
        if (!returnStatus.getStatusCode().equals(TStatusCode.SRM_SUCCESS)) {
            throw new IOException(returnStatus.getExplanation() + " ("
    ...
    public void setPermissions(String uri, TPermissionType tPermissionType,
        ...
        setPermissionRequest
                .setArrayOfGroupPermissions(arrayOfTGroupPermissions);
    ...
    public static class SRMPosixFile implements PosixFileAttributes {
        ...
        protected SRMPosixFile(TMetaDataPathDetail tMetaDataPathDetail)
                throws IOException {
            ...
            ownerTPermissionMode = tMetaDataPathDetail.getOwnerPermission()
                    .getMode();
            ...
            groupTPermissionMode = tMetaDataPathDetail.getGroupPermission()
                    .getMode();
        ...
        public long creationTime() {
            return (tMetaDataPathDetail.getCreatedAtTime() != null ? tMetaDataPathDetail
                    .getCreatedAtTime().getTimeInMillis() : -1L);
        }
        ...
        public long size() {
            return (tMetaDataPathDetail.getSize() != null ? tMetaDataPathDetail
                    .getSize().longValue() : -1L);
        }
