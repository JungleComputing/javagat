...
public class LfcConnector {
     ...
     * @throws IOException
     */
    public LFCFile stat(String path, boolean followSymbolicLink)
            throws IOException {
        final LfcConnection connection = new LfcConnection(server, port,
                proxyPath);
        final LFCFile file;
    ...
    public boolean canRead(String path) throws IOException {
        ...
        try {
            ...
            if (e instanceof ReceiveException) {
                if (((ReceiveException) e).getError() == 13) { // Permission
                                                               // denied
                    return false;
                }
            }
    ...
    public boolean exist(String path) throws IOException {
        ...
        try {
            ...
            if (e instanceof ReceiveException) {
                if (((ReceiveException) e).getError() == 2) { // No such file or
                                                              // directory
                    return false;
                }
            }
     ...
     *             if anything goes wrong
     */
    public Collection<LFCFile> list(String path, boolean followSymbolicLink)
            throws IOException {
    ...
    /**
     * Delete a file or a directory from the LFC. If it's a file, delete also
     * all the replicas from the DPMs
     * 
     * @param path
    ...
    public boolean deletePath(String path) throws IOException {
        if (this.stat(path, true).isDirectory()) {
            ...
            for (LFCReplica replica : replicas) {
                ...
                final LfcConnection connection2 = new LfcConnection(server,
                        port, proxyPath);
                try {
                ...
                try {
                    ...
                    LOGGER.warn("Failed to delete Replica " + replica.getSfn(),
                            e);
                }
            }
            if (LOGGER.isInfoEnabled()) {
    ...
    public void chown(String path, boolean recursive,
            boolean followSymbolicLinks, String usrName, String grpName)
            throws IOException {
        ...
        if (usrName != null) {
            if (usrName.equals("root")) {
                ...
                try {
                    ...
                    throw new IOException("Unable to find the uid of "
                            + usrName + " in the LFC");
                } finally {
                    connection.close();
                }
        ...
        if (grpName != null) {
            if (grpName.equals("root")) {
                ...
                try {
                    ...
                    throw new IOException("Unable to find the gid of "
                            + grpName + " in the LFC");
                } finally {
                    connection.close();
                }
     ...
     */
    public void chown(String path, boolean recursive,
            boolean followSymbolicLinks, int new_uid, int new_gid)
            throws IOException {
        if (recursive == true) {
            // TODO ....
        ...
        if (new_uid < 0 && new_gid < 0) {
            throw new IllegalArgumentException(
                    "You must specify at least a new owner or a new group");
        }
        LfcConnection connection = new LfcConnection(server, port, proxyPath);
        try {
     ...
     *             if anything goes wrong
     */
    public String create(URI location, String guid, long fileSize)
            throws IOException {
