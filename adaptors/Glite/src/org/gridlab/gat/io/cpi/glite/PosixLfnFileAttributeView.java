...
public class PosixLfnFileAttributeView extends AbstractPosixFileAttributeView {
    ...
    public PosixLfnFileAttributeView(URI location, boolean followSymbolicLinks,
            LfcConnector lfcConnector, GATContext gatContext) {
    ...
    public void setGroup(GroupPrincipal group) throws IOException {
        ...
        lfcConnector.chown(location.getPath(), false, followSymbolicLinks,
                null, group.getName());
        if ("true".equalsIgnoreCase((String) gatContext.getPreferences().get(
                GliteConstants.PREFERENCE_SYNCH_LFC_DPM_PERMS))) {
            GATContext srmGatContext = LfcUtil.getSRMContext(gatContext);
            SrmConnector srmConnector = new SrmConnector(
                    GliteSecurityUtils.getProxyPath(srmGatContext));
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    new PosixSrmFileAttributeView(new URI(uri),
                            followSymbolicLinks, srmConnector, srmGatContext)
                            .setGroup(group);
                } catch (Exception e) {
                    if (log == null) {
    ...
    public void setPermissions(Set<PosixFilePermission> perms)
        ...
        for (PosixFilePermission perm : perms) {
            ...
            switch (perm) {
            case OWNER_READ:
                mode |= LfcConnection.S_IRUSR;
                break;
            case OWNER_WRITE:
                mode |= LfcConnection.S_IWUSR;
                break;
            case OWNER_EXECUTE:
                mode |= LfcConnection.S_IXUSR;
                break;
            case GROUP_READ:
                mode |= LfcConnection.S_IRGRP;
                break;
            case GROUP_WRITE:
                mode |= LfcConnection.S_IWGRP;
                break;
            case GROUP_EXECUTE:
                mode |= LfcConnection.S_IXGRP;
                break;
            case OTHERS_READ:
                mode |= LfcConnection.S_IROTH;
                break;
            case OTHERS_WRITE:
                mode |= LfcConnection.S_IWOTH;
                break;
            case OTHERS_EXECUTE:
                mode |= LfcConnection.S_IXOTH;
                break;
            }
        }
    ...
    public void setTimes(Long lastModifiedTime, Long lastAccessTime,
            Long createTime, TimeUnit unit) throws IOException {
