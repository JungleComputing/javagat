...
/**
 * Inspired by the geclipse-project (http://www.geclipse.eu) The
 * VomsProxyManager can be used to create a voms proxy and store it on the
 * filesystem. First, a temporary globus proxy is created using jglobus
 * libraries, then a secure socket to the voms server is established over which
 * VOMS AC requests and responses are passed. The information from the VOMS
 * response is saved as an AC in the globus certificate which can then be stored
 * on the hard-disk.
 * 
 * @author thomas
...
public class VomsProxyManager extends GlobusProxyManager {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(VomsProxyManager.class);
     ...
     * @throws GSSException
     */
    public VomsProxyManager(String userCertFile, String userKeyFile,
            String keyPassword, int lifetime) throws IOException,
    ...
    public VomsProxyManager(String userCertFile, String userKeyFile,
            String keyPassword, int lifetime, String hostDN, String serverURI,
            int serverPort) throws IOException, GeneralSecurityException,
            GSSException {
    ...
    private void connectToVomsServer() throws GATInvocationException {
        ...
        try {
            ...
            GlobusGSSCredentialImpl gsci = new GlobusGSSCredentialImpl(
                    proxyCred, GlobusGSSCredentialImpl.INITIATE_AND_ACCEPT);

            // connect with globus mechanism oid
            ExtendedGSSContext egssContext = (ExtendedGSSContext) this.gssMan
                    .createContext(null, GSSConstants.MECH_OID, gsci,
                            this.lifetime);
            ...
            throw new GATInvocationException(
                    "Problem with the credentials detected!", e);
        } catch (IOException e) {
            throw new GATInvocationException(
                    "Could not open socket at the VOMS server!", e);
        }
     ...
     * @param requestCode
     *            the requested FQANs
     * @return the AttributeCertificate which is returned from the server as a
     *         response
     * @throws IOException
     *             If there are problems with the socket
     */
    private AttributeCertificate query(String requestCode)
            throws GATInvocationException {
        ...
        try {
            ...
            VomsServerCommunicator comm = new VomsServerCommunicator(
                    requestCode, this.lifetime);
            ...
            if (response.hasErrors()) {
                ...
                throw new GATInvocationException(
                        "The server response has errors!");
            }
            ...
            throw new GATInvocationException(
                    "Could not get streams from VOMS-server socket!", e);
        }
    ...
    /**
     * Gets the lifetime of the proxy stored at path. If no proxy exists, -1 is
     * returned as the lifetime
     * 
     * @param path
     *            The path which points to the proxy for which to determine the
     *            remaining lifetime
    ...
    /**
     * Get the string representation of the full qualified attribute names of a
     * VOMS proxy
     * 
     * @param path
     *            The path which points to the proxy
     * @return The list of fqans of the voms proxy or <code>null</code> if the
    ...
    /**
     * Save the managed proxy to a proxy file and set the permissions
     * accordingly
     * 
     * @param path
     *            The path at which the proxy should be stored
     * @throws IOException
    ...
    public void saveProxyToFile(String path) throws IOException, GSSException {
        ...
        try {
            ...
            LOGGER.info("Could not set r/o for proxy: " + path, io);
        }
        outstream.flush();
        outstream.close();
    }
