...
/**
 * Implements the <code>NotifyCallback</code> class, the
 * <code>deliver</code> method is called, when the gstate of
 * the job changed.
 **/
...
public class WSGT4Job extends JobCpi {
	...
	URL factoryURL = null;
	try {
	    factoryURL = ManagedJobFactoryClientHelper.getServiceURL(contactString).getURL();
	...
	submissionID = "uuid:" + org.apache.axis.components.uuid.UUIDGenFactory.getUUIDGen().nextUUID();
    ...
    private void populateJobDescriptionEndpoints(EndpointReferenceType pfactoryEndpoint) throws GATInvocationException {
	EndpointReferenceType[] delegationFactoryEndpoints
            = fetchDelegationFactoryEndpoints(pfactoryEndpoint);
	
	EndpointReferenceType delegationEndpoint
            = delegate(delegationFactoryEndpoints[0], 
		       this.limitedDelegation.booleanValue());
	this.jobDescriptionType.setJobCredentialEndpoint(delegationEndpoint);
        //separate delegation not supported
        this.jobDescriptionType.setStagingCredentialEndpoint(
            delegationEndpoint);
        //delegate to RFT
	...
	fetchDelegationFactoryEndpoints(EndpointReferenceType factoryEndpoint)
	throws GATInvocationException {
	...
	try {
	    epr2 = (EndpointReferenceType) 
		ObjectDeserializer.toObject(any[1], EndpointReferenceType.class);
	} catch(DeserializationException e) {
	    throw new GATInvocationException("WSGT4Job: " + e);
	}
    ...
    private ManagedJobFactoryPortType 
	getManagedJobFactoryPortType(EndpointReferenceType factoryEndpoint)
        throws Exception {
        ManagedJobFactoryPortType factoryPort =
	...
	createJobEndpoint(ManagedJobFactoryPortType factoryPort)
        throws Exception {
	...
	if(gstate.equals(StateEnumeration.CleanUp)
	   || gstate.equals(StateEnumeration.StageOut) ) {
	    setState(this.POST_STAGING);
	}
	if(gstate.equals(StateEnumeration.StageIn)) {
	    setState(this.PRE_STAGING);
	}
	...
	this.fault = fault;
	System.err.println("errcode: " + fault.getTypeDesc());
	...
	request.setResourceProperty(new QName[] {
        ...
        GetMultipleResourcePropertiesResponse response =
            jobPort.getMultipleResourceProperties(request);
	    ...
	    && (any.length==(exitCodeIndex+1))) {
            Integer exitCodeWrapper
                = (Integer) ObjectDeserializer.toObject(any[exitCodeIndex], Integer.class);
    ...
    private void 
	populateStagingDescriptionEndpoints(EndpointReferenceType pfactoryEndpoint,
    ...
    throws GATInvocationException {
        ...
        if ((stageOut != null) || (stageIn != null) || (cleanUp != null)) {
            ...
            factoryAddress = 
		factoryAddress.replaceFirst("ManagedJobFactoryService",
