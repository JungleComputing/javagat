...
public class GT4GridFTPFileAdaptor extends GT4FileAdaptor {
	...
	throws GATInvocationException {
	if(GATEngine.DEBUG) {
	    System.err.println("GT4GridFileAdaptor file: start file copy with destination provider "+ destProvider);
	} 
	...
	System.out.println("gt4 gridftp copy: " + srcProvider + " "
			   + location 
			   + " -> " + dest);
	...
	if(dest.isLocal()) {
	    if(GATEngine.DEBUG) {
		System.err.println("GT4GridFTPFileAdaptor: copy remote to local");
	    }
	    copyToLocal(dest);
	    return;
	}
