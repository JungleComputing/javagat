...
public class GliteResourceBrokerAdaptor extends ResourceBrokerCpi {
    ...
    public Job submitJob(AbstractJobDescription abstractDescription,
        ...
        if (isDryRun(description)) {
            GliteJob job = new GliteJob(gatContext, description, null);
        ...
        try {
            if (preStageFiles.size() > 0) {
                ...
                for (String sandboxURI : sandboxURIs) {
                    ...
                    try {
                        ...
                        Sandbox sandbox = new Sandbox(gatContext, description,
                                uri.getHost(), "/" + uri.getPath(), false,
                                false, false, false);
                        job = new GliteJob(gatContext, description, sandbox);
                ...
                job = new GliteJob(gatContext, description, null);
                job.setClient(client);
