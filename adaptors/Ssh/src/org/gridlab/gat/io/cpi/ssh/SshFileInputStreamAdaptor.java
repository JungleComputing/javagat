...
public class SshFileInputStreamAdaptor extends FileInputStreamCpi {
	...
	protected static Logger logger = Logger
			.getLogger(SshFileInputStreamAdaptor.class);
	...
	public SshFileInputStreamAdaptor(GATContext gatContext, URI location)
	...
	protected InputStream createStream() throws GATInvocationException {
		try {
			...
			try {
				sui = SshSecurityUtils.getSshCredential(gatContext, "ssh",
						location, SshFileAdaptor.SSH_PORT);
			} catch (Exception e) {
	...
	public int available() throws GATInvocationException {
		/*
		 * try { return inputStream.available(); } catch (IOException e) { throw
		 * new GATInvocationException("SshFileInputStream", e); }
		 */
