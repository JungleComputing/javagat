...
public class SshFileAdaptor extends FileCpi {
	 ...
	 */
	public SshFileAdaptor(GATContext gatContext, URI location)
	...
	protected void prepareSession(URI loc) throws GATObjectCreationException,
		...
		try {
			sui = SshSecurityUtils.getSshCredential(gatContext, "ssh", loc,
					SSH_PORT);
		} catch (Exception e) {
	...
	public void copy(URI loc) throws GATInvocationException {
		...
		try {
			target = GAT.createFile(gatContext, loc).getFileInterface();
	...
	protected void doMultipleUpload(URI loc) throws GATInvocationException {
		try {
			FileInterface target = GAT.createFile(gatContext, loc)
					.getFileInterface();
	...
	public org.gridlab.gat.io.File[] listFiles() throws GATInvocationException {
		...
		for (int i = 0; i < r.length; i++) {
			try {
				res[i] = GAT.createFile(gatContext, new URI(uri + r[i]));
			} catch (Exception e) {
				doException(e);
			}
		}
