...
public class LocalFileAdaptor extends FileCpi {
	...
	public LocalFileAdaptor(GATContext gatContext, URI location)
		...
		logger.debug("location before modification: " + location);
		// we have a host name, which means that this file is relative to $HOME
		if (location.getHost() != null && !location.hasAbsolutePath()) {
			logger.debug("location before modification (2): " + location);
			logger.debug("modifying, because host is not null ('"
					+ location.getHost()
					+ "') and location "+location+"is not absolute (absolute="
					+ location.getPath() + ")");
