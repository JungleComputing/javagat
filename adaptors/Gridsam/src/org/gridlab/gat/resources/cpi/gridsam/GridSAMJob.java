    ...
    public class GridSAMJob extends JobCpi {
        ...
        private boolean isFinishedState(JobState jobState) {
            return jobState == JobState.DONE || jobState == JobState.TERMINATED
        ...
        private int translateState(JobState jobState) {
            if (jobState == JobState.ACTIVE) {
                ...
                return PRE_STAGING;
            } else if (jobState == JobState.STAGED_OUT
                    || jobState == JobState.STAGING_OUT) {
                return POST_STAGING;
        ...
        public synchronized Map<String, Object> getInfo()
            ...
            m.put("submissiontime", state == INITIAL ? null
                    : Long.toString(submissionTime));

        m.put("starttime", state == INITIAL || state == SCHEDULED ? null
                : Long.toString(startTime));

        m.put("hostname", state == RUNNING ? adaptor.getBroker().getHost()
                : null);

        m.put("stoptime", state == STOPPED ? Long.toString(stopTime) : null);
