...
public class KoalaJob extends JobCpi implements Runnable {
	...
	protected KoalaJob(GATContext context, JobDescription jobDescription,
			InetSocketAddress schedulerAddress, String schedulingPolicy,
			String rsl) throws GATInvocationException {

		super(context, jobDescription, null);
		...
		if (attributes.containsKey("koala.transfer.optimize")) {
			try {
				boolean optTransfer = (Boolean) attributes
						.get("koala.transfer.optimize");

				if (optTransfer) {
					schedulingPolicy = "BWF";
				}
			} catch (Exception e) {
				logger.warn("Illegal \"koala.transfer.optimize\" setting!");
			}
	...
	/**
	 * Used in synchronisation. the Runners are supposed to wait until they are
	 * assigned job number by the Scheduler
	 */
	...
	private boolean checkJobTryNumber(String jobTryNumber) {
		/*
		 * try { int receivedJobTryNumber = Integer.parseInt(jobTryNumber); if
		 * (receivedJobTryNumber != jobTries.get()) { // this is an old message,
		 * ignore this. logger.debug("Old message (JT:" + receivedJobTryNumber +
		 * ")"); return false; } } catch (NumberFormatException nfe) { //
		 * something wrong with jobtrynumber logger.error("Error in
		 * jobTryNumber"); return false; }
		 */
	...
	private void handleJobRestart(String[] message) {
		...
		/*
		 * synchronized (transfLock) { jobTries.incrementAndGet(); if ((null !=
		 * schedMsg[1]) && (!(schedMsg[1].trim()).equals(""))) {
		 * logger.fatal("Received restart message from Koala server: " +
		 * schedMsg[3].trim()); }
		 * 
		 * failureOccurred.set(true); serverPreemptedUs.set(true);
		 * componentRestart.set(true); stopComponents(); if (flexibleJob.get()) {
	...
	private void handleResetComponents() {
		...
		/*
		 * 
		 * synchronized (runnerLock) { componentsList.clear();
		 * 
		 * jobSize = RslCommon.addRunnerComponent(componentsList, rsl, jobNo);
		 * 
		 * compCount = componentsList.size(); logger.debug("Reset components " +
		 * jobSize + " RSL " + rsl); if (flexibleJob.get()) {
		 * componentsAdded.set(false); } }
		 */
	}
	...
	private void handleSetJobID(String[] message) throws IOException {
		...
		components.createComponent(jobDescription, gatContext.getPreferences(),
				jobNo);

		thread.setName("KoalaJob<" + jobNo + ">");

		/*
		 * synchronized (runnerLock) { componentsList.clear(); jobSize =
		 * RslCommon.addRunnerComponent(componentsList, rsl, jobNo); compCount =
		 * componentsList.size(); if (!flexibleJob.get()) { logger.debug("Waking
		 * up FILE_TRANSFER thread "); componentsAdded.set(true);
		 * runnerLock.notifyAll(); } }
		 * 
		 * logger.info("Koala server has assigned us Job ID " + jobNo + " Total
		 * Job Components " + compCount); // start the job prephase if
		 * (!prePhase()) { stopComponents(); logger.fatal("Prephase was not
		 * successfull, aborting job " + jobNo); send(schedulerAddress,
		 * "JOB_ABORT#" + jobNo + "#" + jobTries.get()); cleanUpRunner(); }
		 */
	}
	...
	private void handleCloneComponent(String[] message) {
		...
		if (message.length < 4) {
			logger.warn("Incorrect server response: "
		...
		try {
			...
			logger.debug("Splitting component " + component + " to " + clones
					+ " components");
		...
		/*
		 * 
		 * synchronized (runnerLock) { String componentNo = schedMsg[1]; int
		 * cloneCount = Integer.valueOf(schedMsg[3]) .intValue();
		 * logger.debug("Splitting component " + componentNo + " to " +
		 * cloneCount + " Components" + " Original Component Count: " +
		 * componentsList.size());
		 * 
		 * for (int j = 0; j < componentsList.size(); j++) { RunnerJobComponent
		 * component = componentsList .get(j); if
		 * (component.getComponentNo().equals(componentNo)) { compCount--;
		 * componentsList.remove(j); for (int i = 0; i < cloneCount; i++) {
		 * RunnerJobComponent tmpcomp = (RunnerJobComponent) component .clone();
		 * tmpcomp.setComponentNo(jobNo + "&007"); tmpcomp.execSites = null;
		 * tmpcomp.execSites = new ArrayList<String>();
		 * componentsList.add(tmpcomp); } } } if (compCount < 1) { compCount =
		 * componentsList.size(); int i = 1; for (RunnerJobComponent component :
		 * componentsList) { component.setComponentNo(jobNo + "&" + i);
		 * logger.debug("Added component number " + component.getComponentNo());
		 * i++; } logger.debug("Total Components added " +
		 * componentsList.size()); } if (componentsList.size() > 0) {
		 * componentsAdded.set(true); runnerLock.notifyAll(); } }
		 */
	}
	...
	private void handleTransferFiles(String[] message) {
		...
		/*
		 * failureOccurred.set(false); serverPreemptedUs.set(false);
		 * componentRestart.set(false); JLT = 0;
		 * 
		 * synchronized (transfLock) { waitForJobId();
		 * 
		 * if (schedMsg.length < 4) { logger.error("Incorrect server response:
		 * no execution site received"); logger.debug("Server message: " +
		 * message); }
		 * 
		 * String componentNo = schedMsg[1]; String execSite = schedMsg[3];
		 * logger.info("Execution site for " + componentNo + " = " + execSite);
		 * 
		 * RunnerJobComponent thisComponent = null; List inputFilesRsl = null; //
		 * find job and add filesite information and get inputFiles for
		 * (RunnerJobComponent component : componentsList) { if
		 * (component.getComponentNo().equals(componentNo)) { thisComponent =
		 * component; } else if (componentNo.indexOf(jobNo) != -1) {
		 * logger.debug("The job was splitted ! "); } } // assert if (null ==
		 * thisComponent) { logger.error("Could not find component " +
		 * componentNo); return; }
		 * 
		 * thisComponent.setReserved(true); synchronized
		 * (thisComponent.execSites) { inputFilesRsl =
		 * RslCommon.getInputFiles(thisComponent .getComponentRSL());
		 * 
		 * if (null == inputFilesRsl) thisComponent.setFtm(
		 * Globals.FileTransferMutex.FTM_UNSUBMITTED); else
		 * thisComponent.setFtm( Globals.FileTransferMutex.FTM_IN_TRANSFER);
		 * 
		 * thisComponent.execSites.clear();
		 * thisComponent.execSites.add(execSite);
		 * thisComponent.execSites.notify(); } // Transfer input files if
		 * necessary //this is a big buggy at the moment !! disable it //
	...
	private void handleSubmit(String[] message) {
		...
		/*
		 * if (schedMsg.length < 4) { logger.error("Incorrect server response:
		 * no rsl received"); logger.debug("Server message: " + message); }
		 * 
		 * String compNo = schedMsg[1]; String compRsl = schedMsg[3];
		 * 
		 * if (componentsList.size() == 0) { try { synchronized (runnerLock) {
		 * runnerLock.wait(); } } catch (InterruptedException e) {
		 * e.printStackTrace(); } }
		 * 
		 * if (JLT == 0) { JLT = System.currentTimeMillis(); }
		 * 
		 * synchronized (SubmitLock) { if (!checkJobTryNumber(schedMsg[2])) {
		 * logger.debug(compNo + " Jobtry number changed while in " +
		 * "comp_submit! Ignoring submission request."); return; } // integrate
		 * new rsl into complist and submit for (RunnerJobComponent component :
		 * componentsList) { if (component.getComponentNo().equals(compNo)) {
		 * logger.debug("Submitting components for execution " +
		 * componentsList.size() + " exec site " + component.execSites.size());
		 * synchronized (component.execSites) { while
		 * (component.execSites.size() == 0) { try { component.execSites.wait(); }
		 * catch (InterruptedException e) { e.printStackTrace(); } } }
		 * 
		 * synchronized (component.getFtm()) { if (haveInputFiles) { // if
		 * ftm_transferred than we dont need // two-phase commit, if
		 * ftm_unsbmitted // means // this component didn't have input // files
		 * so no two-phase commit if (component.getFtm() !=
		 * Globals.FileTransferMutex.FTM_TRANSFERRED && component.getFtm() !=
		 * Globals.FileTransferMutex.FTM_UNSUBMITTED) { // add twoPhase stuff to
		 * rsl with // jst compRsl += "( two_phase = 600 )"; } }
		 * 
		 * component.setComponentRSL(compRsl);
		 * component.setStatus(Status.COMPONENT_PENDING);
		 * 
		 * logger.info("Submitting job component " + component.getComponentNo() + " (" +
		 * component.execSites.get(0) + ")"); int gramResult =
		 * submitComponent(component);
		 * 
		 * if (gramResult != Globals.GRAMJOB_SUCCESS) { logger.error("GRAM
		 * error: " + Assist.getGramErrorString(gramResult) + " (" + gramResult +
		 * ")"); // for two-phase commits component
		 * .setFtm(Globals.FileTransferMutex.FTM_ERROR); if
		 * (!failureOccurred.getAndSet(true)) { // check // if a failure already
		 * occurred if one already occurred than this // job will be canceled
		 * anyway and this // message is old a failure occurred for the first
		 * time (of this jobtry)! jobTries.incrementAndGet(); //
		 * Deactivator.deactivateAll(); component
		 * .setStatus(Status.COMPONENT_FAIL);
		 * 
		 * if (GramErrorHandler .needUpdateErrorCount(gramResult)) {
		 * FailOver.jobFailed(componentsList); }
		 * 
		 * if (GramErrorHandler .canRestartJob(gramResult)) {
		 * componentRestart.set(true); stopComponents(); logger.info("Restarting
		 * job " + jobNo); send(schedulerAddress, "JOB_ERROR#" + jobNo + "#" +
		 * (jobTries.get()-1)); } else { componentRestart.set(false);
		 * stopComponents(); logger.fatal("Aborting job " + jobNo);
		 * send(schedulerAddress, "JOB_ABORT#" + jobNo + "#" +
		 * (jobTries.get()-1)); postPhase(false); cleanUpRunner(); } } break; } //
	...
	private void handleReport(String[] schedMsg) {
		 ...
		 * 
		 * logger.debug("Server message: " + message); } else {
	...
	public void stateChange(int state, long time) {
		...
		GATEngine.fireMetric(this, new MetricEvent(this, Job
				.getStateString(state), statusMetric, time));
	}
}
