...
public class GT42InitialContext implements Context {
     ...
     * @since 1.3
     */
    protected GT42InitialContext(boolean lazy) throws NamingException {
	if (!lazy) {
	    init(null);
	}
    }
     ...
     * @throws	NamingException if a naming exception is encountered
     */
    public GT42InitialContext(Hashtable<?,?> environment)
	...
	if (myProps.get("java.naming.factory.initialGT42") != null) {
    ...
    public static <T> T doLookup(Name name)
        throws NamingException {
        return (T) (new GT42InitialContext()).lookup(name);
    }
