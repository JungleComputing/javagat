...
/**
 * This class supply methods for voms proxy related security issues.
 * 
 * @author Stefan Bozic
 */
public class VomsSecurityUtils {
	...
	/** The Logger */
	private static final Logger LOGGER = LoggerFactory.getLogger(VomsSecurityUtils.class);
	...
	private static GlobusCredential createVOMSProxy(GATContext context) throws GATInvocationException {
		// Must be a unique ID!
		final String vo = (String) context.getPreferences().get(PREFERENCE_VIRTUAL_ORGANISATION);
		final String voServerUrl = (String) context.getPreferences().get(
				PREFERENCE_VIRTUAL_ORGANISATION_SERVER_URL);
		final String voHostDN = (String) context.getPreferences().get(
				PREFERENCE_VIRTUAL_ORGANISATION_HOST_DN);
		final String voServerPort = (String) context.getPreferences().get(
				PREFERENCE_VIRTUAL_ORGANISATION_SERVER_PORT);
		...
		GSSCredential myProxyCred = GlobusSecurityUtils.getCredentialFromMyProxyServer(context, myProxyHost,
	 ...
	 * @param vomsProxyPath the path of the voms proxy
	 */
