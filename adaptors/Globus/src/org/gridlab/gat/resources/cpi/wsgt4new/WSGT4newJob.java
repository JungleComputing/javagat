...
public class WSGT4newJob extends JobCpi implements GramJobListener {
	...
	protected WSGT4newJob(GATContext gatContext, JobDescription jobDescription, Sandbox sandbox) {
		...
		statusMetricDefinition = new MetricDefinition("job.status", MetricDefinition.DISCRETE, "JobState", null, null,
				returnDef);
	 ...
	 */
	public WSGT4newJob(GATContext gatContext, SerializedJob sj) throws GATObjectCreationException {
		...
		try {
			credential = GlobusSecurityUtils.getGlobusCredential(gatContext, "ws-gram", hostUri,
	...
	protected synchronized void setState(JobState state) {
		...
		if (this.state != state) {
			this.state = state;
			MetricEvent v = new MetricEvent(this, state, statusMetric, System.currentTimeMillis());
	...
	public synchronized Map<String, Object> getInfo() throws GATInvocationException {
		...
		m.put("exitvalue", (getState() != JobState.STOPPED && getState() != JobState.SUBMISSION_ERROR) ? null : ""
				+ getExitStatus());
	...
	private void doStateChange(StateEnumeration newState) {
		...
		} else if (jobState.equals(StateEnumeration.Unsubmitted) && state != JobState.PRE_STAGING) {
	...
	public String marshal() {
		...
		synchronized (this) {
			...
			sj = new SerializedJob(getClass().getName(), jobDescription, sandbox, submissionID, submissiontime,
					starttime, stoptime);
		}
	...
	public static Advertisable unmarshal(GATContext context, String s) throws GATObjectCreationException {
		...
		SerializedJob sj = (SerializedJob) GATEngine.defaultUnmarshal(SerializedJob.class, s, WSGT4newJob.class
				.getName());
