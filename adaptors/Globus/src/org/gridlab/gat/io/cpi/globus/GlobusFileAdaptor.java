...
public abstract class GlobusFileAdaptor extends FileCpi {
	 ...
	 */
	protected FTPClient createClient(URI hostURI) throws GATInvocationException, InvalidUsernameOrPasswordException {
	 ...
	 * 
	 * @param hostURI The uri of the FTP host with the given preferences.
	 */
	protected abstract FTPClient createClient(GATContext gatContext, Preferences additionalPreferences, URI hostURI) throws GATInvocationException, InvalidUsernameOrPasswordException;

	/**
	 * Create an FTP Client for "long" interactions (like file copy, streaming).
	 * 
	 * @param hostURI The uri of the FTP host with the given preferences.
	 */
	protected FTPClient createLongClient(GATContext gatContext, Preferences additionalPreferences, URI hostURI) throws GATInvocationException, InvalidUsernameOrPasswordException {
		return createClient(gatContext, additionalPreferences, hostURI);
	} // protected FTPClient createLongClient(GATContext gatContext, Preferences additionalPreferences, URI hostURI) throws GATInvocationException, InvalidUsernameOrPasswordException
	 ...
	 */
	protected void destroyClient(FTPClient c, URI hostURI, Preferences preferences) throws CouldNotInitializeCredentialException, CredentialExpiredException, InvalidUsernameOrPasswordException {
	 ...
	 * for instance, put the client back in a cache.
	 */
	protected abstract void destroyClient(GATContext context, FTPClient c, URI hostURI, Preferences preferences) throws CouldNotInitializeCredentialException, CredentialExpiredException, InvalidUsernameOrPasswordException ;

	/**
	 * Destroy a "short" client that was created with a createClient call. 
	...
	protected void destroyLongClient(GATContext context, FTPClient c, URI hostURI, Preferences preferences) throws CouldNotInitializeCredentialException, CredentialExpiredException, InvalidUsernameOrPasswordException {
		destroyClient(c, hostURI, preferences);
	} // protected void destroyLongClient(GATContext context, FTPClient c, URI hostURI, Preferences preferences) throws CouldNotInitializeCredentialException, CredentialExpiredException, InvalidUsernameOrPasswordException
	...
	throws GATInvocationException {
		...
		try {
			srcClient = createLongClient(gatContext, additionalPreferences, src);
			destClient = createLongClient(gatContext, additionalPreferences, dest);
			...
			if (destClient != null) {
				destroyLongClient(gatContext, destClient, dest, additionalPreferences);
			}
	...
	public boolean createNewFile() throws GATInvocationException {
		...
		try {
			...
			destroyClient(client, dest, null);
		}
		return true;
	...
	throws GATInvocationException {
		...
		try {
			...
			FileInterface destFile = GAT.createFile(gatContext, dest).getFileInterface();
			...
			if (gatContext.getPreferences().containsKey("file.create")) {
				if (((String) gatContext.getPreferences().get("file.create")).equalsIgnoreCase("true")) {
					destFile = GAT.createFile(gatContext, dest).getFileInterface();
			...
			if (gatContext.getPreferences().containsKey("file.chmod")) {
				try {
					client = createLongClient(gatContext, null, dest);
					setImage(client);
			...
			if (client != null) {
				destroyLongClient(gatContext, client, dest, gatContext.getPreferences());
			}
		}
	}
	...
	protected void copyToLocal(URI src, URI dest) throws GATInvocationException {
		...
		try {
			...
			client = createLongClient(gatContext, null, src);
			setImage(client);
	...
	static void chmod(FTPClient client, String path, GATContext gatContext) {
		try {
			Reply r = client.site("CHMOD " + gatContext.getPreferences().get("file.chmod") + " " + path);
			if (r.getMessage().startsWith("250")) {
				if (logger.isDebugEnabled()) {
					logger.debug("CHMOD " + gatContext.getPreferences().get("file.chmod") + " " + path + " successful");
				}
			} else {
				if (logger.isDebugEnabled()) {
					logger.debug("CHMOD " + gatContext.getPreferences().get("file.chmod") + " " + path + " failed (no exception)");
				}
			}
		} catch (Exception e) {
			if (logger.isDebugEnabled()) {
				logger.debug("CHMOD " + gatContext.getPreferences().get("file.chmod") + " " + path + " failed (exception: " + e + ")");
			}
		}
	}
	...
	public long lastModified() throws GATInvocationException {
		...
		try {
			...
			if (client != null)
				destroyClient(client, toURI(), null);
		}
	}
	...
	private org.gridlab.gat.io.FileInfo[] listFileInfo(boolean fiddle) throws GATInvocationException {
		...
		FTPClient client = null;
		client = createClient(gatContext, null, toURI());
		try {
			setActiveOrPassive(client, gatContext.getPreferences());
			if (fiddle) {
				try {
					...
					logger.debug("failed to fiddle with the active/passive settings for the list operation: " + e);
				}
			}
			if (getPath() != null) {
				Vector<FileInfo> list = null;
			...
			if (null != client) {
				destroyClient(gatContext, client, toURI(), gatContext.getPreferences());
			}
		}
		return null;
	...
	protected static void setActiveOrPassive(FTPClient c, Preferences preferences) throws GATInvocationException {
