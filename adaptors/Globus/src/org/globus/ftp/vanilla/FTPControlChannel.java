...
/**
 * <p>
 * Represents FTP Protocol Interpreter. Encapsulates control channel
 * communication.
 * 
 * </p>
 */
public class FTPControlChannel extends BasicClientControlChannel {
    ...
    private static Log logger = LogFactory.getLog(FTPControlChannel.class
            .getName());
    ...
    /**
     * Using this constructor, you can initialize an instance that does not talk
     * directly to the socket. If you use this constructor using streams that
     * belong to an active connection, there's no need to call open()
     * afterwards.
     */
     ...
     * Before returning, it intercepts the initial server reply(-ies), and not
     * positive, throws UnexpectedReplyCodeException. After returning, there
     * should be no more queued replies on the line.
     * 
    ...
    public void open() throws IOException, ServerException {
        ...
        if (hasBeenOpened()) {
            throw new IOException(
                    "Attempt to open an already opened connection");
        }
        ...
        if (!haveStreams()) {
            ...
            socket = new Socket();
            socket.connect(new InetSocketAddress(host, port), 5 * 1000); // 5
                                                                            // seconds
                                                                            // timeout
     ...
     * </ol>
     * If maxWait == WAIT_FOREVER, never timeout and only check conditions (1)
     * and (3).
     * 
     * @param maxWait
     *                timeout in miliseconds
     * @param ioDelay
     *                frequency of polling the control channel and checking the
     *                conditions
     * @param aborted
    ...
    public void waitFor(Flag aborted, int ioDelay, int maxWait)
        ...
        /*
         * A bug in the server causes it to append \0 to each reply. As the
         * result, we receive this \0 before the next reply. The code below
         * handles this case.
         * 
         */
        ftpIn.mark(2);
     ...
     */
    public Reply read() throws ServerException, IOException,
    ...
    /**
     * Write the command to the control channel, block until reply arrives and
     * return the reply. Before calling this method make sure that no old
    ...
    /**
     * Write the command to the control channel, block until reply arrives and
     * check if the command completed successfully (reply code 200). If so,
