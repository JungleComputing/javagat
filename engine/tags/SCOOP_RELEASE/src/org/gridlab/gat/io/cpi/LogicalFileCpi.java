...
public abstract class LogicalFileCpi implements LogicalFile, Monitorable {
	...
	public static Advertisable unmarshal(GATContext context,
		...
		SerializedLogicalFile f = (SerializedLogicalFile) GATEngine.defaultUnmarshal(
				SerializedFile.class, s);
		try {
			LogicalFile lf = GAT.createLogicalFile(context, preferences, f.getName(), f.getMode());
			for(int i=0; i<f.getFiles().size(); i++) {
				lf.addURI(new URI(URIEncoder.encodeUri((String)f.getFiles().get(i))));
			}
			return lf;
		} catch (Exception e) {
