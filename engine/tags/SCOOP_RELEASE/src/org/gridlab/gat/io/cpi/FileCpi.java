...
public abstract class FileCpi implements File {
	...
	public File getParentFile() throws GATInvocationException {
		try {
			...
			return GAT.createFile(gatContext, preferences, new URI(URIEncoder.encodeUri(uri)));
		} catch (Exception e) {
	...
	public File[] listFiles() throws GATInvocationException {
		...
		try {
			...
			for (int i = 0; i < f.length; i++) {
                ...
                uri += f[i];
				res[i] = GAT.createFile(gatContext, preferences, new URI(URIEncoder.encodeUri(uri)));
			}
			return res;
		} catch (Exception e) {
			throw new GATInvocationException("file cpi", e);
	...
	public String[] list(FilenameFilter filter) throws GATInvocationException {
		...
		try {
			...
			for (int i = 0; i < l.length; i++) {
				if (filter.accept(GAT.createFile(gatContext, new URI(URIEncoder.encodeUri(location.getPath()))),
						l[i])) {
					v.add(l[i]);
				}
			}
	...
	public File[] listFiles(FileFilter filter) throws GATInvocationException {
		...
		try {
			...
			Vector v = new Vector();
			for (int i = 0; i < l.length; i++) {
				if (filter.accept(GAT.createFile(gatContext, new URI(URIEncoder.encodeUri(l[i].getPath()))))) {
					v.add(l[i]);
				}
			}
	...
	protected static URI fixURI(URI in, String destScheme) {
		try {
			...
			if (scheme == null) {
				return new URI(URIEncoder.encodeUri(destScheme + ":///" + s));
			} else if (scheme.equals(destScheme)) {
				return in;
			} else {
				int index = s.indexOf(':');
				return new URI(URIEncoder.encodeUri(destScheme + ":"
	...
	protected static void copyDirectory(GATContext gatContext,
		...
		for (int i = 0; i < files.length; i++) {
			...
			try {
				...
				newDest = new URI(URIEncoder.encodeUri(newDestString));
			} catch (URISyntaxException e) {
				throw new GATInvocationException("file cpi", e);
			}
	...
	public static Advertisable unmarshal(GATContext context,
		...
		try {
			return GAT.createFile(context, preferences, new URI(URIEncoder.encodeUri(f.getLocation())));
		} catch (Exception e) {
