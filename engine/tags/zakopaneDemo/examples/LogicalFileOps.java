...
/**********************************************************
 * This program shows how to create a logical file, to 
 * associate multiple physical files with it, to query it for
 * associated files, to replicate files and to delete it.
 **********************************************************/
...
public class LogicalFileOps {
	...
	// Check is <3 because there can be multiple src's.
		...
		// Create URI's for logical file and the LogicalFile object.
		logUri = new URI(args[0]);
		logFile = GAT.createLogicalFile(context, logUri);

		// Create and add the possible multiple File object to LogicalFile.
		// 'arg.length-2' to 'remove' <log_file> and <target> from commandline.
		...
		for(int i=0; i < nSize; i++) {
		    ...
		    System.out.println("element " + i + ": " + tmpFile.getPath() );
		}
	    } catch (Exception e) {
		System.err.println("error: " + e);
                e.printStackTrace();
