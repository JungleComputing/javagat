...
public class GATEngine {
     ...
     * order becomes predictable and reproducible.
     */
	...
	while (st.hasMoreTokens()) {
	    ...
	    Map<String, ClassLoader> sharedLoaders = new HashMap<String, ClassLoader>();
	    File f = new File(adaptorRoot, "shared");
	    if (f.isDirectory()) {
		for (File d : f.listFiles()) {
		    if (d.isDirectory()) {
			try {
			    ClassLoader l = loadDirectory(d, superparentLoader,
				    false);
			    sharedLoaders.put(d.getName(), l);
			} catch (Throwable e) {
			    logger.warn(
				    "Got exception when loading shared directory "
					    + d, e);
			}
		    }
		}
		try {
		    ClassLoader l = loadDirectory(f, superparentLoader, false);
		    sharedLoaders.put("default", l);
		} catch (Throwable e) {
		    logger.warn("Got exception when loading shared directory "
			    + f, e);
		    sharedLoaders.put("default", superparentLoader);
		}
	    }
	    readJarFiles(adaptorRoot, sharedLoaders);
	}
     ...
     * the classloader that loaded the GATEngine class is not a good candidate
     * because this probably is just the system classloader. A better candidate
     * might be the classloader of the class that prompted the loading of
     * javaGAT in the first place, or the context classloader.
     * 
    ...
    protected void readJarFiles(File adaptorRoot,
	    Map<String, ClassLoader> sharedLoaders) {
	...
	// Sort the list of directories, to make sure that the order is
	// deterministic.
	...
	for (File adaptorDir : adaptorDirs) {
	    try {
		ClassLoader sharedLoader = sharedLoaders.get("default");
		for (String s : sharedLoaders.keySet()) {
		    if (new File(adaptorDir, s).exists()) {
			sharedLoader = sharedLoaders.get(s);
			break;
		    }
		}
    ...
    private ClassLoader loadDirectory(File adaptorDir,
	    ClassLoader parentForDir, boolean mustContainAdaptorJar)
	    throws Exception {
	...
	if (logger.isTraceEnabled()) {
	    logger.trace("URLs used for the URL class loader constructed from: "
		    + adaptorDir);
	    if (urls != null) {
		for (URL url : urls) {
		    logger.trace("\t" + url);
	...
	synchronized (this) {
		...
		// Find out which adaptors actually have unmarshallers, if we
		// have
		// not done this yet.
	...
	for (int i = 0; i < unmarshallers.size(); i++) {
		...
		if (logger.isDebugEnabled()) {
		    logger.debug(
			    "unmarshaller for " + c.getName() + " failed:",
		...
		if (logger.isDebugEnabled()) {
		    logger.debug(
			    "unmarshaller for " + c.getName() + " failed:", e);
		}
		// ignore and try next unmarshaller
	    }
	}
	...
	try {
	    String[] result = (String[]) m.invoke((Object) null,
		    (Object[]) null);
	    if (logger.isInfoEnabled()) {
		logger.info("Supported schemes of class " + clazz.getName()
	...
	if (!((SerializedBase) a).checkClassname(expectedClass)) {
	    throw new Error("could not unmarshal object: found "
		    + ((SerializedBase) a).getClassname() + " instead of "
		    + expectedClass);
	}
	return a;
    }
	...
	// Try and terminate jobs before ending adaptors. Changed order.
	// --Ceriel
	try {
	    JobCpi.end();
	} catch (Throwable e) {
	...
	if (gatContext.getPreferences().containsKey(cpiName + ".adaptor.name")) {
	    ...
	    adaptors = reorderAdaptorList(adaptors, cpiName,
