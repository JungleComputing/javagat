...
public class Sandbox {
	...
	/**
	 * Constant for the attribute "sandbox.root" in the software description.
	 * The value defines the path to the root sandbox directory. Default is
	 * generated.
	 */
	public static final String SANDBOX_ATTR_ROOT = "sandbox.root";

	/**
	 * Constant for the attribute "sandbox.useroot" in the software description.
	 * The value defines if the sandbox uses the root defined root directory
	 * (see also {@link #SANDBOX_ATTR_ROOT}). Possible value "true" and "false".
	 * If true the sandbox uses the root directory, otherwise a subdirectory is
	 * created. Default is false.
	 */
	public static final String SANDBOX_ATTR_USE_ROOT = "sandbox.useroot";

	/**
	 * Constant for the attribute "sandbox.delete" in the software description.
	 * Defines if the sandbox will be deleted after job has finished. Possible
	 * value "true" and "false". If true the sandbox is deleted after the job
	 * has finished and the poststaging is done.
	 */
	public static final String SANDBOX_ATTR_DELETE = "sandbox.delete";

	/**
	 * Constant for the attribute "sandbox.prestage.stdin" in the software
	 * description. Defines if the stdin file will be prestaged before job will
	 * be submitted. Possible values are "true" and "false". Default is definied
	 * while creating this sandbox (see
	 * {@link #Sandbox(GATContext, JobDescription, String, String, boolean, boolean, boolean, boolean)}
	 * )
	 */
	public static final String SANDBOX_ATTR_PRESTAGE_STDIN = "sandbox.prestage.stdin";

	/**
	 * Constant for the attribute "sandbox.poststage.stdout" in the software
	 * description. Defines if the stdout file will be poststaged after job has
	 * finished. Possible values are "true" and "false". Default is definied
	 * while creating this sandbox (see
	 * {@link #Sandbox(GATContext, JobDescription, String, String, boolean, boolean, boolean, boolean)}
	 * )
	 */
	public static final String SANDBOX_ATTR_POSTSTAGE_STDOUT = "sandbox.poststage.stdout";

	/**
	 * Constant for the attribute "sandbox.poststage.stderr" in the software
	 * description. Defines if the stderr file will be poststaged after job has
	 * finished. Possible values are "true" and "false". Default is definied
	 * while creating this sandbox (see
	 * {@link #Sandbox(GATContext, JobDescription, String, String, boolean, boolean, boolean, boolean)}
	 * )
	 */
	public static final String SANDBOX_ATTR_POSTSTAGE_STDERR = "sandbox.poststage.stderr";
	...
	public Sandbox(GATContext gatContext, JobDescription jobDescription, String authority, String sandboxRoot,
			boolean createSandboxDir, boolean preStageStdin, boolean postStageStdout, boolean postStageStderr)
		...
		if (sd != null) {
			...
			if (attr != null) {
				sandboxRootPref = (String) attr.get(SANDBOX_ATTR_ROOT);
				sandboxUseRootPref = (String) attr.get(SANDBOX_ATTR_USE_ROOT);
				sandboxDeletePref = (String) attr.get(SANDBOX_ATTR_DELETE);
				sandboxPrestageStdInPref = (String) attr.get(SANDBOX_ATTR_PRESTAGE_STDIN);
				sandboxPoststageStdOutPref = (String) attr.get(SANDBOX_ATTR_POSTSTAGE_STDOUT);
				sandboxPoststageStdErrPref = (String) attr.get(SANDBOX_ATTR_POSTSTAGE_STDERR);
			}
		}
		...
		pre = new PreStagedFileSet(gatContext, jobDescription, authority, sandbox, preStageStdin);
		post = new PostStagedFileSet(gatContext, jobDescription, authority, sandbox, postStageStdout, postStageStderr);

		if (sd != null) {
			wipePreStaged = sd.wipePreStaged();
			wipePostStaged = sd.wipePostStaged();
			toWipe = new PostStagedFileSet(gatContext, sd.getWipedFiles(), authority, sandbox);

			deletePreStaged = sd.deletePreStaged();
			deletePostStaged = sd.deletePostStaged();
			toDelete = new PostStagedFileSet(gatContext, sd.getDeletedFiles(), authority, sandbox);
		}
	...
	private void createSandboxDir(String authority) throws GATInvocationException {
		...
		for (int i = 0; i < 10; i++) {
			...
			try {
				...
				FileInterface f = GAT.createFile(gatContext, sandboxURI).getFileInterface();
				if (f.mkdir()) {
					return;
				}
			} catch (Exception e) {
	...
	public void retrieveAndCleanup(JobCpi j) {
		...
		if (deleteSandboxDir) {
			if (logger.isInfoEnabled()) {
				logger.info("removing sandbox dir: " + "any://" + authority + "/" + sandbox);
			}
			try {
				removeSandboxDir();
			} catch (GATInvocationException e) {
			...
			if (logger.isInfoEnabled()) {
				logger
						.info("removing sandbox dir done "
								+ (removeSandboxException == null ? "(SUCCESS)" : "(FAILURE)"));
			}
		} else {
			if (logger.isInfoEnabled()) {
				logger.info("sandbox persists at: " + "any://" + authority + "/" + sandbox);
			}
		}
	...
	/**
	 * @param authority the authority to set (authority part of a URI, hostname
